...
| G31 | **Invoicing plugin** — proper (VAT) invoices for service businesses and B2B sales: customer details + VAT numbers, sequential invoice numbering (legal requirement, separate from receipt numbers), print/PDF/e-receipt delivery, credit notes riding the refund flow | Every accounting tool; POS systems rarely do it | `page`+`receipt_template` plugin (per Farshid: "should be a plugin if possible" — it can be: sales/customers via existing tables, layout via the print doc model); pairs with G24 accounting push + appointments-plugin verticals |
